...
class OrcaCLI(cmd.Cmd):
    ...
    After analysis, use 'chat' to enter interactive mode.
    """
    prompt = "orca> "
    ...
    def do_save(self, arg):
        ...
        try:
            ...
            serializable_state = {
                ...
                "dynamic_analysis_results": self.workflow_state.get("dynamic_analysis_results"),
                "capabilities": self.workflow_state.get("capabilities"),
                "malware_analysis": self.workflow_state.get("malware_analysis"),
                "completed_steps": self.workflow_state.get("completed_steps")
            }
    ...
    def do_find_string(self, arg):
        """
        Find code references to a specific string in the binary
        Usage: find_string <string_to_search>
        Example: find_string "CreateFile"
        """
        if not arg:
            print("Error: Please provide a string to search for.")
            print("Usage: find_string <string_to_search>")
            return
        ...
        # Use chatbot's string search functionality
        response = self.chatbot._handle_string_search(f'find string "{arg}"')
        print(f"\n{response}")
    
    def do_find_api(self, arg):
        """
        Find code references to a specific API in the binary
        Usage: find_api <api_name>
        Example: find_api CreateFile
        """
        if not arg:
            print("Error: Please provide an API name to search for.")
            print("Usage: find_api <api_name>")
            return
        ...
        # Use chatbot's API search functionality
        response = self.chatbot._handle_api_search(f'find api "{arg}"')
        print(f"\n{response}")
    
    def do_analyze_function(self, arg):
        """
        Analyze a specific function in the binary
        Usage: analyze_function <function_name>
        Example: analyze_function main
        """
        if not arg:
            print("Error: Please provide a function name to analyze.")
            print("Usage: analyze_function <function_name>")
            return
        ...
        # Use chatbot's function analysis functionality
        response = self.chatbot._handle_function_analysis(f'analyze function "{arg}"')
        print(f"\n{response}")
    ...
    def do_save_conversation(self, arg):
        ...
        try:
            self.chatbot.save_conversation(arg)
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="ORCA Multi-Agentic Workflow CLI")
